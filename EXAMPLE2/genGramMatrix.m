%% ========================================================================
%  FUNCTION TO GENERATE MONOMIAL MOMENT MATRIX FOR DD-GPCE BASIS
%  FUNCTION
...
nA = 1; 
for ii = 1:SS
    nA = nA + nchoosek(NN,ii)*nchoosek(mm,ii);
end
%% LOAD SAMPLES     
nSample = 500;
xx = zeros(nSample,1);
for ii = 1:nSample
FILENAME_LF = sprintf('C:/Work/2022SMO_DD-GPDD/example2/multifidelity/LF2/output%d.rpt',ii);
...
xx(ii) = max(outputData_LF(:,2));
fclose all;
end 

FilNam = sprintf('LF_MOMENT_MATRIX_%d_%d.mat',SS,mm); % OUTPUT DATA 
